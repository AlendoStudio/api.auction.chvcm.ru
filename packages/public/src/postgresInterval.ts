...
export class PostgresInterval implements Interval {
  ...
  public static fromPostgresOutput(value?: string): PostgresInterval {
    ...
    interval.milliseconds =
      (parseInt((matches[12] || "0").padEnd(6, "0"), 10) / 1000) * isNegative;
  ...
  public toPostgresOutput(): string {
    return [
      ...
      [
        [
          [this.hours, this.minutes, this.seconds, this.milliseconds].some(
            (x) => x < 0,
          )
            ? "-"
            : "",
  ...
  public toJSON(): Interval {
    ...
    Object.entries(interval).map(
      (pair) => !pair[1] && delete interval[pair[0]],
    );
    return interval;
  }
}
