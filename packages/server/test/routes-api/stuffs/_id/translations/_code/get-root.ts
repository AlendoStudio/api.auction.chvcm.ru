...
describe("GET /stuffs/:id/translations/:code", () => {
  prepareApi();
  ...
  it("Success 200", async () => {
    ...
    await supertest(Web.instance.app)
      .get(`${Const.API_MOUNT_POINT}/stuffs/1/translations/en`)
      .set("Authorization", `Bearer ${token}`)
      .expect(200, {
        data: {
          code: "en",
  ...
  it("Bad Request 400", async () => {
    await supertest(Web.instance.app)
      .get(`${Const.API_MOUNT_POINT}/stuffs/aaa/translations/en`)
      .set("Authorization", `Bearer ${token}`)
      .expect(400, {
    ...
    await supertest(Web.instance.app)
      .get(`${Const.API_MOUNT_POINT}/stuffs/1/translations/russian`)
      .set("Authorization", `Bearer ${token}`)
      .expect(400, {
        code: ApiCodes.BAD_REQUEST,
  ...
  it("Not Found 404", async () => {
    await supertest(Web.instance.app)
      .get(`${Const.API_MOUNT_POINT}/stuffs/999/translations/en`)
      .set("Authorization", `Bearer ${token}`)
      .expect(404, {
    ...
    await supertest(Web.instance.app)
      ...
      .expect(404, {
        ...
        message: "stuff translation with same code not found",
      });
  });
});
