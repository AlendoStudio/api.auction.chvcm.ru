...
describe("GET /lots/:id", () => {
  ...
  it("Success 200", async () => {
    await supertest(Web.instance.app)
      .get(`${Const.API_MOUNT_POINT}/lots/1`)
      .set("Authorization", `Bearer ${tokenEntity}`)
      .expect(200, {
        data: {
          amount: "2.25",
    ...
    await supertest(Web.instance.app)
      .get(`${Const.API_MOUNT_POINT}/lots/2`)
      .set("Authorization", `Bearer ${tokenEmployee}`)
      .expect(200, {
        data: {
          amount: "100",
  ...
  it("Bad Request 400", async () => {
    await supertest(Web.instance.app)
      .get(`${Const.API_MOUNT_POINT}/lots/aaa`)
      .set("Authorization", `Bearer ${tokenEmployee}`)
      .expect(400, {
  ...
  it("Not Found 404", async () => {
    await supertest(Web.instance.app)
      .get(`${Const.API_MOUNT_POINT}/lots/999`)
      .set("Authorization", `Bearer ${tokenEmployee}`)
      .expect(404, {
        code: ApiCodes.LOT_NOT_FOUND_BY_ID,
        message: "lot with same id not found",
      });
  });
});
